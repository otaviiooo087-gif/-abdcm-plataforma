...
import { X, UploadCloud, Download, CheckCircle2, AlertCircle, Upload, Loader2, XCircle, Sparkles } from 'lucide-react';
...
type Stage = 'upload' | 'preview' | 'importando' | 'resumo';
...
export const ImportarListaModal: React.FC<ImportarListaModalProps> = ({
  ...
  const [arquivosDocPorKey, setArquivosDocPorKey] = useState<Map<string, { mimeType: string; nomeArquivo: string }>>(new Map());
  ...
  const processarPlanilha = async (file: File) => {
    ...
    setErro(null);
    setStage('importando');
    setMensagemProgresso('Enviando e conferindo a planilha...');
    setProgresso({ feitos: 0, total: 0 });
  ...
  const handleLerDocumentosAutomaticamente = async () => {
    if (arquivosDoc.length === 0) return;
    setErro(null);
    setMensagemProgresso('Enviando os documentos...');
    setProgresso({ feitos: 0, total: arquivosDoc.length });
    ...
    const worker = async () => {
      while (fila.length > 0) {
        ...
        try {
          ...
          feitos++;
          setProgresso({ feitos, total: arquivosDoc.length });
        }
      }
    };
    ...
    setMensagemProgresso('Lendo os documentos com IA...');
    try {
      ...
      setArquivosDocPorKey((prev) => new Map([...prev, ...porKey]));
  ...
  const handleConfirmarImport = async () => {
    setStage('importando');
    setMensagemProgresso('Importando a lista...');
    setProgresso({ feitos: 0, total: 0 });
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          {stage === 'importando' && (
            <div className="flex flex-col items-center gap-3 py-8">
              ...
              <p className="text-xs font-semibold text-slate-600">{mensagemProgresso}</p>
              {progresso.total > 0 && (
                  ...
                  <div className="border border-slate-200 rounded-xl divide-y divide-slate-100 max-h-32 overflow-y-auto">
                    ...
                    <div className="px-3 py-2">
                      <button
                        ...
                        className="w-full py-1.5 text-xs font-bold text-white bg-[#148296] hover:bg-[#0f6b7c] rounded-lg cursor-pointer transition-colors"
                      >
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {stage === 'preview' && (
              ...
              >
                Importar {linhasOk.length} Nome(s)
              </button>
            )}
            {(stage === 'upload' || stage === 'resumo') && (
              <button
                type="button"
                onClick={handleClose}

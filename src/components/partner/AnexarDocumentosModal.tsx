...
import { X, FolderUp, FileCheck2, AlertTriangle, CheckCircle2, XCircle, Loader2, Sparkles, Upload } from 'lucide-react';
...
interface AssociadoLite {
  id: string;
  nome: string;
  cpf_cnpj: string;
}

type Modo = 'pastas' | 'ocr';
...
function agruparArquivos(fileList: File[]): { encontrados: ArquivoEncontrado[]; ignorados: number } {
...
export const AnexarDocumentosModal: React.FC<AnexarDocumentosModalProps> = ({ isOpen, onClose, onConcluido }) => {
  ...
  const [arquivosOcr, setArquivosOcr] = useState<ArquivoOcr[]>([]);
  const [arquivosOcrPorKey, setArquivosOcrPorKey] = useState<Map<string, File>>(new Map());
  const [ocrItens, setOcrItens] = useState<OcrItemLocal[]>([]);
  const [falhasStaging, setFalhasStaging] = useState<string[]>([]);
  const [associadosLista, setAssociadosLista] = useState<AssociadoLite[]>([]);
  const [lendoComIa, setLendoComIa] = useState(false);
  ...
  const handlePastaSelecionada = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const arquivosClonados = Array.from(files);
    e.target.value = '';

    const { encontrados, ignorados: qtdIgnorados } = agruparArquivos(arquivosClonados);
  ...
  const handleRemoverArquivoOcr = (id: string) => {
    setArquivosOcr((prev) => prev.filter((a) => a.id !== id));
  };

  const handleLerAutomaticamente = async () => {
    if (arquivosOcr.length === 0) return;
    setEtapa('conferindo');
    setErro(null);
    ...
    const worker = async () => {
      while (fila.length > 0) {
        ...
        try {
          ...
          feitos++;
          setProgresso({ feitos, total: arquivosOcr.length });
        }
      }
    };

    await Promise.all(Array.from({ length: Math.min(CONCORRENCIA_UPLOAD, arquivosOcr.length) }, () => worker()));

    if (itensStaged.length === 0) {
      setErro(`Nenhum arquivo pôde ser enviado. ${falhasLocais.join(' ')}`);
      setEtapa('selecionar');
      return;
    }
    ...
    setLendoComIa(true);
    try {
      const previewRes = await fetch('/api/documentos/ocr/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ itens: itensStaged }),
      });
      const previewData = await previewRes.json();
      if (!previewRes.ok) throw new Error(previewData.error || 'Falha ao ler os documentos');

      const associadosRes = await fetch('/api/associados');
      const associadosData: AssociadoLite[] = associadosRes.ok ? await associadosRes.json() : [];

      setArquivosOcrPorKey(porKey);
  ...
  const handleToggleIncluirOcr = (key: string) => {
    setOcrItens((prev) => prev.map((it) => (it.key === key ? { ...it, incluir: !it.incluir } : it)));
  };
  ...
  const handleConfirmarEnvioOcr = async () => {
    setEtapa('enviando');
    setProgresso({ feitos: 0, total: itensOcrParaEnviar.length });
    const falhasLocais: string[] = [];
    let feitos = 0;
    let sucessoLocal = 0;

    const fila = [...itensOcrParaEnviar];
    const worker = async () => {
      while (fila.length > 0) {
        const item = fila.shift();
        if (!item) return;
        const file = arquivosOcrPorKey.get(item.key);
        try {
          const confirmRes = await fetch('/api/documentos/confirmar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          falhasLocais.push(`${item.nomeArquivo}: ${err instanceof Error ? err.message : 'erro desconhecido'}`);
        } finally {
          feitos++;
          setProgresso({ feitos, total: itensOcrParaEnviar.length });
        }
      }
    };

    await Promise.all(Array.from({ length: Math.min(CONCORRENCIA_UPLOAD, itensOcrParaEnviar.length) }, () => worker()));
              ...
              <div className="grid grid-cols-2 gap-1.5 p-1 bg-slate-100 rounded-xl">
                <button
                  ...
                  className={`py-2 rounded-lg text-xs font-bold transition-colors cursor-pointer ${
                    modo === 'pastas' ? 'bg-white text-[#148296] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  Pastas por CPF
                </button>
                <button
                  type="button"
                  onClick={() => setModo('ocr')}
                  className={`py-2 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center justify-center gap-1.5 ${
                    modo === 'ocr' ? 'bg-white text-[#148296] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  <Sparkles className="w-3.5 h-3.5" /> Leitura automática (IA)
                </button>
              </div>
...
{`documentos/
  ...
  98765432100/
    ...
    rg.jpg`}
          ...
          {etapa === 'conferindo' && (
            <div className="flex flex-col items-center gap-3 py-8 text-slate-500">
              ...
              {modo === 'pastas' ? (
                ...
                <p className="text-xs font-semibold">Lendo os documentos com IA...</p>
              ) : (
                <>
          ...
          {etapa === 'preview' && modo === 'ocr' && (
            <>
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                  <p className="text-xl font-black text-emerald-700">{ocrItens.filter((i) => i.autoConfirmavel).length}</p>
                  <p className="text-[10px] text-emerald-800 font-semibold">lidos com confiança alta</p>
                </div>
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                  <p className="text-xl font-black text-amber-700">
                    {ocrItens.filter((i) => !i.autoConfirmavel && i.associadoIdSugerido).length}
                  </p>
                  <p className="text-[10px] text-amber-800 font-semibold">precisam de conferência</p>
                </div>
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-3">
                  <p className="text-xl font-black text-rose-700">{ocrItens.filter((i) => !i.associadoIdSugerido).length}</p>
                  <p className="text-[10px] text-rose-800 font-semibold">sem associado encontrado</p>
                </div>
              </div>

              {falhasStaging.length > 0 && (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 max-h-24 overflow-y-auto space-y-1">
                  <p className="text-[11px] font-bold text-rose-800">
                    {falhasStaging.length} arquivo(s) não puderam ser enviados:
                  </p>
                  {falhasStaging.map((f, i) => (
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-2.5 shrink-0">
          {etapa === 'selecionar' && modo === 'ocr' && arquivosOcr.length > 0 && (
            <button
              type="button"
              onClick={handleLerAutomaticamente}
            ...
            >
              Ler {arquivosOcr.length} Documento(s) Automaticamente
            </button>
          )}
          ...
          {etapa === 'preview' && modo === 'ocr' && (
            <>
              ...
              <button
                type="button"
                disabled={itensOcrParaEnviar.length === 0}
                onClick={handleConfirmarEnvioOcr}
                className="px-5 py-2 text-xs font-bold text-white bg-[#148296] hover:bg-[#0f6b7c] rounded-lg shadow-sm cursor-pointer transition-colors disabled:opacity-50"
              >
                Confirmar e Anexar {itensOcrParaEnviar.length} Documento(s)
              </button>
            </>
          )}
